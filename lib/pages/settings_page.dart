...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _exportData() async {
    ...
    if (includeKeys == null || !mounted) return;
    ...
    try {
      ...
      if (mounted) _showResultSnackBar('导出失败：$e', isSuccess: false);
      return;
    }
    ...
    if (saveLocation != null) {
      try {
        ...
        if (mounted) _showResultSnackBar('保存失败：$e', isSuccess: false);
        return;
      }
      if (mounted) {
        _showResultSnackBar(
  ...
  Future<void> _importData() async {
    ...
    try {
      ...
      if (mounted) {
        _showResultSnackBar('无法打开文件选择器', isSuccess: false);
      }
      return;
    }
    ...
    try {
      ...
      if (mounted) _showResultSnackBar('读取文件失败：$e', isSuccess: false);
      return;
    }
    if (!mounted) return;
    ...
    try {
      ...
      if (mounted) _showResultSnackBar('导入失败：$e', isSuccess: false);
    }
  }
